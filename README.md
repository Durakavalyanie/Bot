...
<start, end>
```
## <span style="color:blue"> 3 Создать событие </span>
Промпт бота:
```
Итак, для того, чтобы создать событие, нужно придумать для него короткое название, которое пользователи будут видеть в афише, указать дату, а также придумать описание.
Не теряя времени, начнём с названия!
```
Следующее сообщение бота содержит меню ответов:
```
Если хочешь задать название, просто напиши его
1. Назад
```
### 3.0
